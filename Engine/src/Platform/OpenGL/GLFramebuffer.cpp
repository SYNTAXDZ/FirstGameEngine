...
namespace FGE {
    ...
    void GLFramebuffer::Invalidate() {
        ...
        FGE_CORE_ASSERT( glCheckFramebufferStatus( GL_FRAMEBUFFER ) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!" );
    ...
    void GLFramebuffer::Resize( uint32_t width, uint32_t height ) {
        ...
        if( width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize ) {
            ...
            FGE_CORE_WARN( "Attempted to rezize framebuffer to {0}, {1}", width, height );
            return;
