...
namespace FGE {
    ...
    static GLenum ShaderTypeFromString( const std::string& type ) {
        ...
        FGE_CORE_ASSERT( false, "Unknonwn Shader Type" );
        return 0;
    ...
    GLShader::GLShader( const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc  ) : m_Name( name ) {
    ...
    std::string GLShader::ReadFile( const std::string& filepath ) {
        ...
        if( in ) {
            ...
            if( size != -1 ) {
                ...
                FGE_CORE_ERROR( "Could not Read From File {0}", filepath );

            }

        } else {

            FGE_CORE_ERROR( "Could not Open The File {0}", filepath );
    ...
    std::unordered_map<GLenum, std::string> GLShader::PreProcess( const std::string& source ) {
        ...
        while( pos != std::string::npos ) {
            ...
            FGE_CORE_ASSERT( eol != std::string::npos, "Syntax error" );
            size_t begin = pos + typeTokenLength + 1; // Start of shader type name (after "#type " keyword)
            std::string type = source.substr( begin, eol - begin );
            FGE_CORE_ASSERT( ShaderTypeFromString( type ), "Invalid shader type specified" );

            size_t nextLinePos = source.find_first_not_of( "\r\n", eol ); // Start of shader code after shader type declaration line
            FGE_CORE_ASSERT( nextLinePos != std::string::npos, "Syntax error" );
    ...
    void GLShader::Compile( const std::unordered_map<GLenum, std::string>& shaderSources ) {
        ...
        FGE_CORE_ASSERT( shaderSources.size() <= 2, "We only support 2 shaders for now" );
        ...
        for( auto &kv : shaderSources ) {
            ...
            if( isCompiled == GL_FALSE ) {
                ...
                FGE_CORE_ERROR( "{0}", infoLog.data() );
                FGE_CORE_ASSERT( false, "Shader compilation failure!" );
        ...
        if( isLinked == GL_FALSE ) {
          ...
          // We don't need the program anymore.
            ...
            FGE_CORE_ERROR( "{0}", infoLog.data() );
            FGE_CORE_ASSERT( false, "Shader link failure!" );
